...
void handle_rf_rx_data(void)
{
    ...
    uprintf(UART0, "RF: ordre d'affichage: %s\n\r", data[2]);
	snprintf(displayOrder, 3, "%s", data[2]);
#endif
...
void uv_config(int uart_num)
{
	...
	if(ret != 0) {
		uprintf(uart_num, "UV config eror: %d\n\r", ret);
	}
}
	...
	while (1) {
		...
		if (update_display == 1) {
			...
			for(int i = 0; i < 3; i++) {
				...
				} else if(displayOrder[i] == 'H') {
					snprintf(data, 20,"Humi: %d,%d rH", humidity/10, humidity%10);
				} else if(displayOrder[i] == 'L') {
					snprintf(data, 20,"Lux: %d", lux);
				}
				if(i == 0) line = 1;
				else if(i == 1) line = 3;
				else line = 5;
				display_line(line, 0, data);
			}
